...
f = cgi.FieldStorage()
phone = f.getvalue('Lphone')
pwd = f.getvalue('Lpwd')
if not phone or not pwd:
    print("Error: Phone or password is missing.")
else:
   
    signup = pymysql.Connect(host="127.0.0.1", user="root", database="doc_pat")
    cursor = signup.cursor()

    query = "SELECT * FROM pat_det WHERE phone = %s"
    cursor.execute(query, (phone,))
    row1 = cursor.fetchone()
    print("first If")
    if row1:
        print("Insert first If")
        query = "SELECT * FROM pat_det WHERE passw = %s"
                ...
                <style>
                    * {
                    ...
                    .heading h1{
                        ...
                        text-align: center;
                        width: 100%;
                        background-color: rgb(154, 183, 226);
                        z-index: 1;
                    }
                    .disease img{
